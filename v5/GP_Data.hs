...
instance Gene CTTerm CTTermGen where generateIt = cttermGen
instance Cros CTTerm CTTermCro where crossIt    = cttermCro
  ...
  case maybe_cPos2 of
   ...
   Just cPos2 -> do
    let sub1          = ttreeSubtree tree1 cPos1
        (tree2',sub2) = ttreeChangeSubtree tree2 cPos2 sub1
        (tree1',_   ) = ttreeChangeSubtree tree1 cPos1 sub2
        tooBig1 = ttreeDepth tree1' > maxDepth
        tooBig2 = ttreeDepth tree2' > maxDepth
        child1  = if tooBig1 then tree1 else tree1'
        child2  = if tooBig2 then tree2 else tree2'
...
-- ZAKOMENTOVÁNO PROTOŽE TO POUŽÍVALO TEN STAREJ INHABITATION MACHINE
