...
uniqueN num = uniqueN' Set.empty num
 where 
  uniqueN' :: Set TTerm -> Int -> Eva TTerm -> Eva [TTerm]
...
kozaProveN n limit typ ctx = 
 ...
 where
  kozaProveOne :: IMGraph -> Limit -> Typ -> Context -> Eva TTerm
...
strategyProveOne' :: Limit -> Strategy -> IMGraph -> Taxi -> Eva (Maybe [Token2])
...
strategyOneNext :: Strategy -> IMGraph -> Taxi -> Eva ( Either [Token2] (Maybe Taxi) )
...
strategyNextTaxis :: Strategy -> IMGraph -> Taxi -> Eva ( Either [Token2] [Taxi] )
...
strategyNextTaxis' :: CurrentDepth -> Strategy -> IMGraph -> NContext -> Typ -> Eva [ ( [Token2] , NContext ) ]
...
randProveOne :: Int -> Typ -> Context -> Eva TTerm
...
randProveOne' :: Limit -> IMGraph -> Taxi -> Eva ( Maybe [Token2] )
...
randOneNext :: IMGraph -> Taxi -> Eva (  Either [Token2] (Maybe Taxi)  )
