...
this method is obtained simply by replacement of 
the first grammar rule by slightly modified rule,
resulting in following grammar.
\begin{align*}
( \pi , \Gamma )  
&\gar
(~f~( \rho_1 , \Gamma )~\dots~( \rho_m , \Gamma )~)
...
The difference lies in that $f$'s type is no longer obligated to be fully expanded
($\pi \in \T$ instead of $\alpha \in A$). This makes the grammar less deterministic,
resulting in bigger search space. The new rule is generalization of the old one,
...
\lterms $M$,$N$ such that $M \not= N$ and $M =_{\eta} N$; 
but by generating in \bnf we do not avoid this.
