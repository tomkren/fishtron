...
Genetic programming (GP) is an AI technique, which falls into 
broader category of evolutionary algorithms --- 
mataheuristic search algorithms inspired by 
biological evolution by natural selection.
I dare say many people perceive its beauty in the fact that GP is
a computer program constructing other computer programs 
%(property sometimes called \textit{automatic programming}) 
with desired properties by breeding them. 
It is also pretty successful technique in a number of areas. 
...
In classical GP \textit{type consistency} of those 
building blocks is required, property also called \textit{closure}.
This means that all functions must accept 

~\\[3em]Témata co zmínit v úvodu :\\

\begin{easylist}[itemize]

& Co je to GP, 
 && že je to část EVA
 && že v clasickym GP je closure
 && a ruzný prace ukazujou jak se ho zbavit

& Co je to typ
  && typ jedobře zaběhlá programátorská konstrukce
  && matematicky uchopuje typy a programy v teorii typovaného lambda kalkulu

& cílem této práce je 
  && ukázat jak spolu hezky souvysej
     &&& např skrzestavební bloky a context
  && a jak využít konstrukty typový teorie k prospěchu v GP
     &&& such as AE, beta,etha reduction,...
  && na základě takto zmapované podobnosti navrhnout systém a implementovat
     rather then build yet another GP system, generalize ten klasickej
